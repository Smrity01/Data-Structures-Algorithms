/*
length of Floyd cycle in linked list
...
template <class T>
int linkedlist<T>::length_cycle(node<T>* slow){
	/*
	Objective       : length of cycle in the Linked list
	Input parameter : None
	Output Values   : None
	Description     : Member function definition
	Approach        : traverse using while loop and incrementing the counter
	*/
	int length = 2;
	node<T>* temp = slow;
...
void linkedlist<T>::floyd_detection(node<T>* head){
	...
	*/
	...
	while(slow && fast && fast->next){
		...
		fast = fast->next->next;
		if(slow == fast){
			cout << "Loop found with length: " << length_cycle(slow);
			return;
		}
	}
	cout << "Loop not found ";
